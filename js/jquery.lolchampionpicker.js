...
(function($) {
    $.fn.lolChampionPicker = function(options) {
        ...
        const lolChampionDataURL = `//ddragon.leagueoflegends.com/cdn/${settings.lolPatchApiVersion}/data/${settings.lang}/champion.json`;

        const renderData = (champions) => {
            $.each(champions, (index, champion) => {
                let html = `<img src="//ddragon.leagueoflegends.com/cdn/${settings.lolPatchApiVersion}/img/champion/${champion.image.full}" height="${settings.image.height}" width="${settings.image.width}" class="off pickable" id="${champion.id}"/>`
                that.append(html);
            });
        };
